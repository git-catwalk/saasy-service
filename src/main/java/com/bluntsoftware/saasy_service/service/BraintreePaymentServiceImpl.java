...
public class BraintreePaymentServiceImpl implements PaymentService {
    ...
    public Optional<Subscription> updateSubscription(String tenantId,SaasySubscription sr) {
        ...
        Subscription subscription = null;
        Optional<Subscription> subscriptionOptional = this.getSubscriptionByTenantId(tenantId);
        if(subscriptionOptional.isPresent()){
            ...
            subscription  = braintreeRepository.gateway().subscription().update(sub.getId(), req).getTarget();
        }else{
            subscriptionOptional = createSubscription(tenant.getId(),sr.getPlanId(),customer);
